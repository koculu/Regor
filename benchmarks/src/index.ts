import { createApp as createRegorApp, html, ref } from '../../src'
...
const runRegor = async (root: HTMLElement): Promise<BenchResult> => {
  ...
  const initialRows = makeRows(ROW_COUNT)

  await nextFrame()
  const mountT0 = performance.now()
  const app = createRegorApp(
    {
      rows: ref(initialRows),
    },
    {
      element: root,
      template: html`<div>
  ...
  const mutateT0 = performance.now()
  for (let round = 0; round < ROUNDS; round++) {
    const rows = app.context.rows()
    for (let i = 0; i < rows.length; i += STRIDE) {
      const row = rows[i]()
      row.value.value += round + 1
      row.active.value = !row.active.value
      row.label.value = `row-${row.id.value}-${round}`
    }
    const mid = (rows.length / 2) | 0
    const inserts = makeRows(SPLICE_SIZE, nextId).map((x) => ref(x))
    nextId += SPLICE_SIZE
    rows.splice(mid, SPLICE_SIZE, ...inserts)
    rows.push(ref(makeRows(1, nextId++)[0]))
    rows.shift()
  }
  await nextFrame()
  const mutateMs = performance.now() - mutateT0
  const totalMs = mountMs + mutateMs
...
const runVue = async (root: HTMLElement): Promise<BenchResult> => {
  ...
  for (let round = 0; round < ROUNDS; round++) {
    ...
    for (let i = 0; i < rows.length; i += STRIDE) {
      ...
      row.label = `row-${row.id}-${round}`
    }
    const mid = (rows.length / 2) | 0
    const inserts = makeRows(SPLICE_SIZE, nextId)
    nextId += SPLICE_SIZE
    rows.splice(mid, SPLICE_SIZE, ...inserts)
    rows.push(makeRows(1, nextId++)[0])
    rows.shift()
  }
  await nextFrame()
  const mutateMs = performance.now() - mutateT0
...
const renderResults = (rows: BenchSummary[]): void => {
  ...
  for (const r of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${formatMs(r.mutateMedian)}</td>
      <td>${formatMs(r.mutateP90)}</td>
      <td>${formatMs(r.totalMedian)}</td>
      <td>${formatMs(r.totalP90)}</td>
      <td>${r.rowsFinal}</td>
      <td>${r.samples}</td>
    `
...
const runOnlyRegor = async (): Promise<void> => {
  ...
  try {
    ...
    setLog('Regor done. Showing median + p90.')
...
const wire = (): void => {
  ...
  void loadVueModule()
    .then(() => {
      setButtonsDisabled(false)
      setLog(
        `Ready. Default run uses ${WARMUP_RUNS} warmups + ${SAMPLE_RUNS} samples (median/p90).`,
      )
    })
    .catch((e) => {
